...
public class SampleRun {
    ...
    public static void main (String[] args) throws Exception {
        EntityManager em = SampleRun.getEntityManager();
    ...
    public static void setupDerbyDatabase() throws Exception {
        EntityManager em = SampleRun.getDerbyEntityManager();
        Stamp stampNew = new Stamp();
        em.getTransaction().begin();
        stampNew.setId("1");
        stampNew.setDescription("Turkish Stamp");
        stampNew.setImage(FileUtils.readFileToByteArray(new File("src/main/config/Turkish_USA_stamp.png")));
        em.persist(stampNew);

        stampNew = new Stamp();
        stampNew.setId("2");
        stampNew.setDescription("Newfoundland Stamp");
        stampNew.setImage(FileUtils.readFileToByteArray(new File("src/main/config/Lord_Bacon_stamp.png")));
        em.persist(stampNew);
        em.getTransaction().commit();
    }

    public static EntityManager getDerbyEntityManager() {
        Properties emfProps = new Properties();
        String driver, url;
        // Starting Derby in Network (multi JVM) mode:
        driver = "org.apache.derby.jdbc.ClientDriver";
        url = "jdbc:derby://localhost:3223/memory:stampdb";

        emfProps.setProperty("javax.persistence.jdbc.driver", driver);
        emfProps.setProperty("javax.persistence.jdbc.url", url);
        emfProps.setProperty("javax.persistence.jdbc.user", "APP");
        emfProps.setProperty("javax.persistence.jdbc.password", "APP");
